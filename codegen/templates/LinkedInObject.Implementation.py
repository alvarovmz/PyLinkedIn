{% if class.identity %}
    @property
    def _identity( self ):
        return {% include "LinkedInObject.Concatenation.py" with concatenation=class.identity only  %}
{% endif %}

    def _initAttributes( self ):
{% for attribute in class.attributes|dictsort:"name" %}
        self._{{ attribute.name }} = LinkedInObject.NotSet
{% endfor %}
...
{% for attribute in class.attributes|dictsort:"name" %}
    ...
    {% with simple_or_complex=attribute.type.simple|yesno:"simple,complex_as_dict" %}
        ...
        {% with template_name="LinkedInObject.IsInstance."|add:attribute.type.cardinality|add:"."|add:simple_or_complex|add:".py" %}
    ...
    {% with simple_or_complex=attribute.type.simple|yesno:"simple,complex" %}
        {% with template_name="LinkedInObject.AttributeValue."|add:simple_or_complex|add:".py" %}
