{% for parameter in method.mandatoryParameters %}
    {% with simple_or_complex=parameter.type.simple|yesno:"simple,complex" %}
        {% with template_name="LinkedInObject.IsInstance."|add:parameter.type.cardinality|add:"."|add:simple_or_complex|add:".py" %}
...
{% for parameter in method.optionalParameters %}
    {% with simple_or_complex=parameter.type.simple|yesno:"simple,complex" %}
        {% with template_name="LinkedInObject.IsInstance."|add:parameter.type.cardinality|add:"."|add:simple_or_complex|add:".py" %}
        assert {{ parameter.name }} is LinkedInObject.NotSet or {% include template_name with variable=parameter.name type=parameter.type only %}, {{ parameter.name }}
        {% endwith %}
    {% endwith %}
{% endfor %}
...
{% if method.variadicParameter %}
    {% with simple_or_complex=method.variadicParameter.type.simple|yesno:"simple,complex" %}
        {% with template_name="LinkedInObject.IsInstance.list."|add:simple_or_complex|add:".py" %}
