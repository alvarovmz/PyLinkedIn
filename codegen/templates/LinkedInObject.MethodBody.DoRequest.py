{% if method.request.postParameters %}
    ...
    {% else %}
        ...
        if {{ parameter.name }} is not LinkedInObject.NotSet:
...
{% if method.request.urlParameters %}
    ...
    {% for parameter in method.request.urlParameters %}
            "{{ parameter.name }}": {% include "LinkedInObject.Concatenation.py" with concatenation=parameter.value only %},
    {% endfor %}
        }
{% endif %}
...
{% endif %}
            "{{ method.request.verb }}",
            {% include "LinkedInObject.Concatenation.py" with concatenation=method.request.url only %},
