...
description = json.load( open( os.path.join( os.path.dirname( __file__ ), "JsonDescriptionOfLinkedInApiV3", "description.normalized.json" ) ) )
...
LinkedInObjectTemplate = django.template.loader.get_template( "LinkedInObject.py" )
for class_ in description[ "classes" ]:
    with open( os.path.join( os.path.dirname( __file__ ), "..", "LinkedIn", class_[ "name" ] + ".py" ), "wb" ) as f:
        f.write( "# WARNING: this file is generated automaticaly.\n" )
        f.write( "# Do not modify it manually, your work would be lost.\n" )
        f.write( "\n" )
        rawCode = LinkedInObjectTemplate.render( django.template.Context( { "class": class_ } ) ).split( "\n" )
        code = list()
        for line in rawCode:
            line = line.rstrip()
            if line == "":
